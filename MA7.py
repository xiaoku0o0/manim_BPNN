...
class BPNN():
    ...
    def back(self, input_ls: list, except_out_ls: list, alpha: float = 0.5, e: float = 1e-2) -> float:
        """误差反向传播"""
        """
        input_ls 输入数据列表
        except_out_ls 预期输出列表
        alpha 学习率
        e 容差
        """
        if len(input_ls) != self.node_numbers_ls[0]:
            raise ValueError(f"数据输入维度{len(input_ls)}与输入层节点个数{self.node_numbers_ls[0]}不匹配")
        if len(except_out_ls) != self.node_numbers_ls[-1]:
            raise ValueError(f"预期输出维度{len(except_out_ls)}与输出层节点个数{self.node_numbers_ls[-1]}不匹配")
            ...
            for args_idx in range(len(args_ls)):
                if wight_idx < len(self.node_ls[layer_idx][node_idx].w_ls):
                    ...
                    if node_idx >= self.node_numbers_ls[layer_idx]: # 该层节点已完成赋值，转向下一层
                        layer_idx += 1
                        node_idx = 0
        def cal_error(now_output_ls: list) -> float:
            """计算误差（损失函数）"""
            return np.linalg.norm(np.array(now_output_ls)-np.array(except_out_ls))
        args_ls = get_args_ls()
        # 梯度下降
        def cal_grad(args_ls,h = 1e-6) -> list:
            ...
            # 计算初始误差
            update_args(args_ls)
            error_init = cal_error(self.update(input_ls))
            for args_idx in range(len(args_ls)):
                args_temp = args_ls[:]
                args_temp[args_idx] += h
                update_args(args_temp)
                grad.append((cal_error(self.update(input_ls)) - error_init)/h)
            return grad
        while np.linalg.norm(np.array(self.update(input_ls))-np.array(except_out_ls)) > e:
        ...
        return cal_error(self.update(input_ls))     # 返回误差
    def train(self, input_data: list, except_out_data: list, turn: int, alpha: float = 0.5, e_one: float = 1e-2, e_all: float = 0.3):
        """训练神经网络"""
        """
        ...
        for times in range(turn):
            ...
            for data_idx in range(data_nums):
                err += self.back(input_data[data_idx],except_out_data[data_idx],alpha,e_one)
...
class MA7(Scene):
    def construct(self):
        ...
        axes_err = Axes(
            ...
            y_length=2,
            y_axis_config={
                "numbers_to_include": np.array([0.1, 0.2])
            },
            tips=False
        ).move_to((-3, -2.3, 0))
        axes_err_label = VGroup(
            Text("迭代次数", font="SimSun", font_size=20).move_to(axes_err.coords_to_point(50, -0.03)),
            Text("平均误差", font="SimSun", font_size=20).rotate(90*DEGREES).move_to(axes_err.coords_to_point(-18, 0.1)),
        )
        self.add(axes_err, axes_err_label)

        # 开始训练神经网络
        def train(input_data: list, except_out_data: list, turn: int, alpha: float = 0.5, e_one: float = 1e-2,
                  e_all: float = 0.3):
            """训练神经网络"""
            """
            input_data 二维列表，每个列表项是输入数据列表
            ...
            def update_times_label(times, err):
                times_label = VGroup(
                    Text(f"迭代第{times:0>3}轮", font="SimSun", font_size=20, color=YELLOW_E),
            ...
            for times in range(turn):
                err = 0
                for data_idx in range(data_nums):
                    err += bpnn.back(input_data[data_idx], except_out_data[data_idx], alpha, e_one)
                    # manim方面
                    svg_temp = load_svg(svg_type=data_out[data_idx][0], xy=data_in[data_idx], fill=True)
                    self.add(svg_temp)
                    ...
                    for dot_obj in act_dot_queue:
                        dot_obj.update()
                    self.wait(0.1)
                    self.remove(svg_temp)
                times_label.become(update_times_label(times+1, err/len(data_in)))
                self.add(Dot(
                    point=axes_err.coords_to_point(times, err/len(data_in)),
                    radius=0.02,
                    color=ORANGE
                ))
            return err
        train(data_in, data_out, 100, e_one=0.1, e_all=0.1)
        self.wait()
